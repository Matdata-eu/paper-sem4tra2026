...
%%
%% Harvey-ball style rating symbols used in the ontology comparison matrix
%% (Table~\ref{tab:ontology-comparison}). #1 is the number of filled quarters
%% (0--4), drawn clockwise from the top.
%%
\newcommand{\ratedisc}[1]{%
  \tikz[baseline=-0.55ex]{%
    \ifnum#1>0
      \fill (0,0) -- (90:0.62ex)
        arc[start angle=90, end angle=\the\numexpr90-90*#1\relax,
            radius=0.62ex] -- cycle;%
    \fi
    \draw (0,0) circle (0.62ex);}}
\newcommand{\ratefull}{\ratedisc{4}}
\newcommand{\ratehigh}{\ratedisc{3}}
\newcommand{\ratehalf}{\ratedisc{2}}
\newcommand{\ratelow}{\ratedisc{1}}
\newcommand{\ratenone}{\ratedisc{0}}
...
lessons, we present a simplified view here and defer the operational plumbing
(orchestration, scheduling, clustering) and performance figures to
Section~\ref{sec:implementation}. The pipeline is mature and runs in production:
it currently produces roughly 13 million triples across seventeen ontologies,
covering more than five thousand kilometres of track~\cite{dim}.
...
It is worth being precise about \emph{what} is being compared. The railway
standards of Section~\ref{sec:topology} fall into two families. RTM~\cite{rtm},
RSM~\cite{rsm} and OpenTNF~\cite{opentnf} are essentially \emph{topology,
positioning and entity-defining} schemas: they establish how the network graph
and locations along it are described, but they deliberately say little about the
assets themselves. railML~3.x~\cite{railml} and the ERA
ontology~\cite{eraontology}, by contrast, are \emph{asset-describing} models that
build on such a topological foundation to catalogue the physical and functional
infrastructure. A meaningful requirements comparison therefore only makes sense
\emph{within} a family; pitting an asset ontology against a pure topology format
such as RTM would not be fair. For that reason we restrict the comparison in
Table~\ref{tab:ontology-comparison} to the two asset-describing models that are
genuine candidates for our backbone: railML~3.x and the ERA ontology.

The single respect in which ERA is clearly ahead is that it is a \emph{native}
ontology with first-class RINF compatibility: it is expressed in OWL/RDF and is
the regulatory target for European infrastructure reporting, whereas railML is
primarily an XML exchange format whose ontology is a more recent, still-maturing
derivative and whose data must be \emph{mapped} to satisfy RINF
(Section~\ref{sec:rinf}). Both models are open and community-governed, although
railML carries some licensing restrictions on its schema. On virtually every
other axis, however, railML is currently the more complete choice for an internal
backbone. It is a widely used exchange format with proven industrial adoption and
a detailed asset catalogue, and it supports a number of capabilities that the ERA
ontology does not yet express at all -- among them:

\begin{itemize}
  \item railML distinguishes over- and undercrossings, where ERA offers only
        generic bridges and tunnels;
  \item railML supports network levels, element aggregation and the description
        of multiple coexisting networks;
  \item railML can convert between topological (intrinsic), linear (kilometric --
        across several linear positioning systems) and geographic coordinates,
        which ERA cannot;
  \item railML describes left/right and through/diverging branches at a switch,
        absent in ERA;
  \item railML models signaling panels (speed indicators, pantograph and
        traction controls, and so on) that have no ERA counterpart;
  \item railML treats overhead contact line, etcs areas, speed sections, gradients 
        and curves as first-class positioned entities, whereas ERA links them to a track;
  \item railML covers interlocking (routes, locks, signal aspects, work zones),
        schematic visualisations and timetabling (itineraries, trains,
        connections, rosterings) -- none of which the ERA ontology currently
        addresses.
\end{itemize}

None of this is a criticism of the ERA ontology's trajectory. It is a young,
regulatory-driven model, and we have every confidence that it will evolve to
overcome these limitations; the authors indeed intend to contribute, in their own
capacity, to that evolution. For the specific role of an \emph{internal backbone}
that must already express the full richness of the infrastructure today, however,
railML remains the pragmatic choice, and we treat the ERA ontology as an export
target (Section~\ref{sec:rinf}).

\begin{table*}[t]
  \centering
  \caption{Requirements matrix comparing the two asset-describing candidate
    ontologies for the internal backbone. See the note below for the rating
    scale.}
  \label{tab:ontology-comparison}
  \begin{tabularx}{\textwidth}{@{}Xcc@{}}
    \toprule
    \textbf{Requirement} & \textbf{railML~3.x} & \textbf{ERA ontology} \\
    \midrule
    Native ontology              & \ratenone & \ratefull \\
    Open and community-governed  & \ratehigh & \ratefull \\
    RINF compatibility           & \ratehalf & \ratefull \\
    Widely used exchange format  & \ratefull & \ratehalf \\
    Proven industrial adoption   & \ratefull & \ratehalf \\
    Detailed railway asset catalog & \ratehigh & \ratelow \\
    Topology management          & \ratelow  & \ratenone \\
    Schematic track plan         & \ratehigh & \ratelow \\
    Network statement            & \ratehigh & \ratehalf \\
    Route time calculation       & \ratehigh & \ratenone \\
    Timetable management         & \ratehigh & \ratenone \\
    ETCS data exchange           & \ratehigh & \ratenone \\
    \bottomrule
  \end{tabularx}

  \smallskip
  {\footnotesize Rating scale (Harvey balls, more filled is stronger):
    \ratefull~full / native support, \ratehigh~good support,
    \ratehalf~emerging or partial support, \ratelow~limited support,
    \ratenone~none or not (yet) available.\par}
\end{table*}
...
%% =====================================================================
\section{Technical Implementation and Performance}
\label{sec:implementation}
%% =====================================================================

The pipeline of Section~\ref{sec:pipeline} deliberately hid the operational
plumbing. We make it concrete here and report execution times, since
reproducibility and performance are legitimate concerns for a production
knowledge graph and were raised in review.

\subsection{Deployment}

The pipeline runs on a managed Kubernetes cluster (Azure Kubernetes Service) as
part of the DIM platform~\cite{dim}. Orchestration is provided by Apache
Airflow~3.2, deployed as the usual set of components -- API server, scheduler,
DAG processor, triggerer and a Celery worker -- backed by a PostgreSQL metadata
database and a Redis (Valkey) broker, with all pipeline state held on a shared
Azure Files volume mounted into every pod. The actual data processing runs in a
single Celery worker pod, which is by far the largest workload: it is
provisioned with 1--4 vCPU and 28--32~GiB of memory. The worker image bundles
the full toolchain -- a Java~21 runtime, Apache Jena~5.6 (TDB2 and
Fuseki)~\cite{jena}, SPARQL Anything~\cite{sparqlanything}, SpatiaLite for the
spatial pre-processing~\cite{spatialite}, and both SHACL engines, TopBraid
SHACL~\cite{topbraidshacl} and maplib~\cite{maplib}. SPARQL Anything is run with
a 16~GB Java heap; splitting and post-processing over the full graph are the main
drivers of the worker's large memory envelope.

\subsection{Orchestration and Reliability}

The pipeline is decomposed into a sequence of Airflow DAGs -- source ingestion,
SPARQL Anything mapping, post-processing, splitting, railML generation and RINF
generation -- chained through Airflow's data-aware scheduling: each stage
declares the dataset it produces, and the next stage is triggered automatically
when that dataset is updated. All post-processing runs against a \emph{staging}
TDB2 dataset that is only hot-swapped behind the stable Fuseki endpoint once the
stage has completed successfully (Section~\ref{sec:pipeline}). Individual tasks
are configured to retry up to three times with exponential backoff, so that
transient failures -- dominated by Azure Files I/O latency and occasional memory
pressure during the splitting stage -- are absorbed automatically, and the
hot-swap guarantees that consumers never observe a partially processed graph even
when a stage fails and is re-run.

\subsection{Execution Times}

Table~\ref{tab:benchmark} reports the per-stage wall-clock time on the hardware
above, for the full production dataset (roughly 13 million triples, more than
five thousand kilometres of track). Two observations stand out. First, the
\emph{splitting} stage (1~h~30) and \emph{post-processing} (45~min) dominate the
end-to-end run; both are pure SPARQL over the whole graph in Fuseki, and the
former is precisely the fifteen sequential topology-traversal queries whose cost
we attribute to the absence of first-class topology support
(Section~\ref{sec:geosparql}). Second, the choice of SHACL engine for the RINF
export matters by an order of magnitude: the same rule set that maplib executes
in about 10~minutes takes TopBraid SHACL roughly 2~hours, which quantifies the
qualitative claim of Section~\ref{sec:discussion}. Excluding the slower TopBraid
alternative, an end-to-end run completes in under three hours.

\begin{table}[t]
  \centering
  \caption{Per-stage wall-clock execution time of the production pipeline
    (Figure~\ref{fig:pipeline}), measured on a single Celery worker pod (1--4
    vCPU, 28--32~GiB) on AKS. The two RINF rows are \emph{alternative} engines
    for the same step, not additive.}
  \label{tab:benchmark}
  \small
  \begin{tabularx}{\columnwidth}{@{}Xr@{}}
    \toprule
    \textbf{Pipeline stage} & \textbf{Time} \\
    \midrule
    Source ingestion (SpatiaLite SQL, CSV export)\textsuperscript{a} & 3.5\,min \\
    SPARQL Anything mapping ($\sim$50 CONSTRUCT) & 10\,min \\
    Post-processing ($\sim$50 SPARQL UPDATE, Fuseki) & 45\,min \\
  ...
  \smallskip
  {\footnotesize \textsuperscript{a}Assumes a warmed-up OpenTNF input whose
    SpatiaLite spatial views are already precomputed.\par}
\end{table}
